...
def train(config, resume, **extra_args):
    ...
    data_loader_class = getattr(module_data, config['data_loader']['type'])#返回对象属性值
    ...
    model1 = getattr(module_arch, config['model1']['type'])
    model2 = getattr(module_arch, config['model2']['type'])
    ...
    optimizer_class1 = getattr(torch.optim, config['optimizer']['type'])
    optimizer1 = optimizer_class1(trainable_params1, **config['optimizer']['args'])

    trainable_params2 = filter(lambda p: p.requires_grad, model2.parameters())
    optimizer_class2 = getattr(torch.optim, config['optimizer']['type'])
    optimizer2 = optimizer_class2(trainable_params2, **config['optimizer']['args'])
    ...
    trainer = trainer_class(config, model, loss, metrics, optimizer, optimizer1,optimizer2,lr_scheduler, resume, data_loader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--device', default=1, type=str, help='indices of GPUs to enable (default: all)')
    ...
    postfix = '_' + module
    module_data = importlib.import_module('.data_loader' + postfix, package='data_loader')#动态导入模块
