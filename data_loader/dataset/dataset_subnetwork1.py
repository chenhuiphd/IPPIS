...
class TrainDataset(Dataset):
    ...
    def __init__(self, data_dir, extra_dir='', transform=None):
        ...
        self.sem_dir = os.path.join(data_dir, 'sem')
    ...
    def __getitem__(self, index):
        ...
        R = np.load(os.path.join(self.R_dir, self.names[index]))
        sem = Image.open(os.path.join(self.sem_dir, self.names[index].split(".")[0]+".png")).convert("RGB")
        #sem = np.array(sem)
        #sem = np.expand_dims(sem, axis=2)
        # (C, H, W)
        ...
        R = torch.tensor(np.transpose(R, (2, 0, 1)), dtype=torch.float32)
        sem = torch.tensor(np.transpose(sem, (2, 0, 1)), dtype=torch.float32)
        ...
        return {'I_alpha': I_alpha, 'I': I, 'delta_I': delta_I, 'P_A': P_A, 'P_T': P_T, 'T': T, 'name': name,'R':R,'sem':sem}
...
class InferDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        I = torch.tensor(np.transpose(I, (2, 0, 1)), dtype=torch.float32)  # [0, 1] float, as float32
        delta_I = torch.tensor(np.transpose(delta_I, (2, 0, 1)), dtype=torch.float32)  # [0, 1] float, as float32
        sem = torch.tensor(np.transpose(sem, (2, 0, 1)), dtype=torch.float32)  # [0, 1] float, as float32
        ...
        return {'I_alpha': I_alpha, 'I': I, 'delta_I': delta_I, 'name': name,'sem':sem}
